<template>
    <el-dialog v-model="defData.visible" draggable title="修改菜单" width="860px">
...
const openDialog = async (row: MenuApi_MenuItem) => {
    ...
    const ids = row.id ? getParentId(props.data, row.id, 'id') as unknown as string[] : []
    ids.pop() // 删除最后一项
    formData.menuPidArr = ids
    const arr = deepClone(props.data)
    defData.routeArr = setDisableTree(arr, row.id)

    formData.menuType = row.menu_type
    formData.id = row.id
    formData.title = row.title
    formData.name = row.title
    formData.path = row.route_path
    formData.redirect = row.redirect
    formData.component = row.page_path
    formData.icon = row.icon
    formData.linkUrl = row.link_url
    formData.isAffix = row.is_affix
    formData.isHide = row.is_hide
    formData.isIframe = row.is_iframe
    formData.isLink = row.is_link
    formData.isKeepAlive = Number(row.is_keep_alive) as 0 | 1
    formData.sort = Number(row.sort)
    // TODO: 菜单列表需要字段api_path
...
const onSubmit = async () => {
    ...
    const menuPid = formData.menuPidArr ? formData.menuPidArr?.join(',') : ''

    const data: MenuApi_Edit = {
        id: formData.id,
    ...
    const res = await ApiFunc(MenuApi.edit(data))
