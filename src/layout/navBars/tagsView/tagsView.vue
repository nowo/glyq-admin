...
const onTagsViewClickBus = useEventBus<0 | 1 | 2 | 3 | 4>('onContextmenuClick')
    ...
    return itemRoute.find((v: any) => {
        if (
            ...
            && isObjectValueEqual(
                ...
                cParams && Object.keys(cParams || {}).length > 0 ? cParams : null,
            )
        ) {
            return v
...
const tagsViewMoveToCurrentTag = () => {
    nextTick(() => {
        ...
        // 当前滚动条的值
        const scrollRefs = scrollbarRef.value?.wrapRef as HTMLDivElement
...
onBeforeMount(() => {
    ...
    /**
     * 当前页操作，  监听非本页面调用
     * @param param  0 刷新当前，1 关闭当前，2 关闭其它，3 关闭全部 4 当前页全屏
     */
    onTagsViewClickBus.on((param) => {
        const dat: any = Object.assign({}, { contextMenuClickId: param, ...route })
        onCurrentContextmenuClick(dat)
    })
    // 监听布局配置界面开启/关闭拖拽
    openOrCloseSortableBus.on(() => {
        initSortable()
    })
    // 监听布局配置开启 TagsView 共用，为了演示还原默认值
    openShareTagsViewBus.on(() => {
        if (getThemeConfig.value.isShareTagsView) {
            router.push('/')
            state.tagsViewList = []
